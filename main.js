...
class MatterbridgeAdapter extends utils.Adapter {
  constructor(options) {
    ...
    // immune to this.
    const dataDir =
      this.config.dataDir || path.join(__dirname, "..", "..", "iobroker-data");
   ...
   * action required.
   *
   * @param mbDataDir Absolute path to the new iobroker-data/matterbridge directory
   */
  _migrateOldLocationIfNeeded(mbDataDir) {
    ...
    if (fs.existsSync(newInstall)) {
      return;
    } // already migrated, or fresh install

    const anyOldExists =
      fs.existsSync(oldInstall) ||
      fs.existsSync(oldStorage) ||
      fs.existsSync(oldNodeRuntime);
    if (!anyOldExists) {
      return;
    ...
    try {
      ...
      this.log.info(
        `Migrated existing Matterbridge installation from ${__dirname} to ${mbDataDir} (protects against npm install/prune during other adapters' updates).`,
      );
    } catch (err) {
      ...
      const logFn = this.log?.error
        ? (msg) => this.log.error(msg)
        : (msg) => console.error(msg);
      logFn(
        `Failed to migrate the Matterbridge installation: ${err.message}. Please move it manually from ${__dirname} to ${mbDataDir}.`,
      );
    }
  }
  ...
  async onReady() {
    ...
    if (!nodeRuntimeInstalled) {
      this.log.info(
        `Isolated Node.js ${this.nodeMajor} runtime for Matterbridge not found - downloading it now (only used for Matterbridge, the system Node.js is left untouched)`,
      );
      try {
        await this.installNodeRuntime();
        await this.setStateAsync("info.nodeRuntimeReady", true, true);
        this.log.info(
          "Node.js runtime for Matterbridge installed successfully",
        );
      } catch (err) {
        this.log.error(
          `Failed to download/install the isolated Node.js runtime: ${err.message}. Matterbridge cannot run reliably without a compatible Node.js version.`,
        );
        return;
      }
    }
    ...
    if (!installed) {
      this.log.info(
        "Matterbridge is not installed yet - installing it now locally via npm",
      );
      try {
        ...
        this.log.error(
          `Installation failed: ${err.message}. Please check manually (e.g. permissions for a global npm installation).`,
        );
        return;
      }
    }
    ...
    try {
      ...
      this.log.error(
        `Could not install the bundled ioBroker bridge plugin: ${err.message}`,
      );
    }
  ...
  async ensureBridgePlugin() {
    ...
    this.log.info(
      `Installing bundled plugin "${pluginName}" (generic ioBroker device bridge)...`,
    );
    ...
    this.log.info(
      `Plugin "${pluginName}" installed and registered (0 devices active for now - selection happens via "whiteList"/"idPrefixes" in the plugin configuration in the Matterbridge frontend).`,
    );
  }
  ...
  async ensureNpmrcPrefix() {
    ...
    try {
      ...
      if (err.code !== "ENOENT") {
        this.log.warn(
          `Could not read ${npmrcPath} (${err.message}) - trying to write it anyway.`,
        );
      }
      lines = [];
    }
    ...
    if (prefixLineIndex === -1) {
      ...
      this.log.info(
        `~/.npmrc: added "prefix=${this.mbInstallDir}" (file: ${npmrcPath}). This ensures plugin installations via Matterbridge's own install button also land in the correct location.`,
      );
      return;
    }
    ...
    this.log.warn(
      `~/.npmrc contained a deviating "prefix=" entry ("${lines[prefixLineIndex].trim()}") - setting it to "${desiredLine}" so Matterbridge plugin installations reliably land in the isolated directory.`,
    );
  ...
  async installNodeRuntime() {
    ...
    if (platform !== "linux" && platform !== "darwin") {
      throw new Error(
        `Automatic Node.js runtime download is not supported on platform "${platform}". Please extract Node.js ${this.nodeMajor}.x into ${this.nodeRuntimeDir} manually.`,
      );
    }
    if (archRaw !== "x64" && archRaw !== "arm64") {
      throw new Error(
        `Automatic Node.js runtime download is not supported on architecture "${archRaw}". Please extract Node.js ${this.nodeMajor}.x into ${this.nodeRuntimeDir} manually.`,
      );
    }
    ...
    this.log.info(
      `Looking up the current Node.js ${this.nodeMajor}.x version for ${tag}...`,
    );
    ...
    const line = shasums
      .split("\n")
      .find(
        (l) => l.trim().endsWith(suffix) && l.includes(`v${this.nodeMajor}.`),
      );
    if (!line) {
      throw new Error(
        `Could not find a matching Node.js ${this.nodeMajor}.x file for ${tag} in SHASUMS256.txt.`,
      );
    }
    ...
    await new Promise((resolve, reject) => {
      exec(
        `tar -xzf "${tmpTarball}" -C "${this.nodeRuntimeDir}" --strip-components=1`,
        (err, stdout, stderr) => {
    ...
    if (!ok) {
      throw new Error(
        "Node.js binary not found/executable after extraction - the download may be incomplete.",
      );
    }
  }
      ...
      https
        .get(url, (res) => {
          if (
            ...
            res.resume();
            return resolve(
              this._httpGetText(res.headers.location, redirectsLeft - 1),
            );
          }
  ...
  installMatterbridge() {
    return new Promise((resolve, reject) => {
      ...
      const registryArg = this.config.npmMirror
        ? ` --registry=${this.config.npmMirror}`
        : "";
      ...
      exec(
        ...
        (err, stdout, stderr) => {
          ...
          this.log.info(
            "Matterbridge installed successfully (user-local global prefix, isolated Node.js runtime)",
          );
          resolve();
        },
      );
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            this.log.warn(
              `Could not remove lock file (${full}): ${err.message}`,
            );
          }
        }
      }
  ...
  _doStartMatterbridge() {
    const args = [
      ...
      "--service",
      "-frontend",
      String(this.config.frontendPort || 8283),
      "-port",
      String(this.config.matterPort || 5540),
      "-homedir",
      this.mbStorageDir,
    ...
    this.child.stdout.on("data", (data) =>
      this.log.debug(`[matterbridge] ${data.toString().trim()}`),
    );
    this.child.stderr.on("data", (data) =>
      this.log.warn(`[matterbridge] ${data.toString().trim()}`),
    );
    ...
    this.child.on("exit", (code, signal) => {
      ...
      if (this.config.autoRestart !== false) {
        this.log.warn(
          `Matterbridge exited (code=${code}, signal=${signal}) - restarting in ${this.backoffMs / 1000}s`,
        );
    ...
    if (id.endsWith("control.installPlugin") && state.val) {
      ...
      if (pluginName) {
        this.installAndAddPlugin(pluginName).catch((err) => {
          this.log.error(
            `Failed to install plugin "${pluginName}": ${err.message}`,
          );
        });
      }
    }
    ...
    if (id.endsWith("control.removePlugin") && state.val) {
      ...
      if (pluginName) {
        this.removePlugin(pluginName).catch((err) => {
          this.log.error(
            `Could not remove plugin "${pluginName}": ${err.message}`,
          );
        });
      }
    }
   ...
   *
   * @param args Matterbridge CLI arguments, e.g. ["-add", pkgPath]
   */
  runMatterbridgeCli(args) {
    return new Promise((resolve, reject) => {
      ...
      exec(
        `"${this.nodeBin}" ${fullArgs.map((a) => `"${a}"`).join(" ")}`,
        {
          maxBuffer: 1024 * 1024 * 10,
          env: {
            ...process.env,
   ...
   *
   * @param pluginName npm package name of the Matterbridge plugin to install
   */
  installAndAddPlugin(pluginName) {
    return new Promise((resolve, reject) => {
      ...
      this.log.info(
        `Installing plugin "${pluginName}" (isolated Node.js/npm version, correct prefix)...`,
      );
      exec(
        cmd,
        {
        ...
        async (err, stdout, stderr) => {
          ...
          this.log.info(
            `Plugin "${pluginName}" installed, registering it with Matterbridge...`,
          );
          try {
            ...
            await this.runMatterbridgeCli(["-add", pkgPath]);
            this.log.info(
              `Plugin "${pluginName}" added successfully. Matterbridge needs to be restarted for it to load.`,
            );
            resolve();
          } catch (addErr) {
            reject(addErr);
  ...
  async removePlugin(pluginName) {
    ...
    this.log.info(
      `Plugin "${pluginName}" removed from Matterbridge (files on disk are kept - delete them manually if needed).`,
    );
  }
